...
class DeviceFactory:
    ...
    def register_device_class(self, device_class):
        ...
        Args:
            device_class: 设备类，必须支持supports_robot_type类方法
        """
        if hasattr(device_class, 'supports_robot_type'):
            self._device_classes.append(device_class)
        else:
            raise ValueError(f"设备类 {device_class.__name__} 必须支持 supports_robot_type 类方法")
    
    def create_device_for_robot_type(self, robot_type, send_message_callback=None, **kwargs):
        """
        根据robot_type创建设备实例
        ...
        for device_class in self._device_classes:
            if device_class.supports_robot_type(robot_type):
                device = device_class(send_message_callback=send_message_callback, **kwargs)
                device.set_robot_type(robot_type)
                return device
...
class HexDeviceApi:
    ...
    def __init__(self, ws_url: str, control_hz: int = 1000):
        ...
        self.__shutdown_event = None  # 在初始化时就创建
    ...
    def _register_available_device_classes(self):
        ...
        # 可以在这里添加更多设备类的注册
        # try:
        #     from .other_device import OtherDevice
        #     self.register_device_class(OtherDevice)
        #     log_info("已注册 OtherDevice 设备类")
        # except ImportError as e:
        #     log_warn(f"无法导入 OtherDevice: {e}")
    ...
    def find_device_by_robot_type(self, robot_type) -> Optional[DeviceBase]:
        ...
        for device in self.device_list:
            if hasattr(device, 'robot_type') and device.robot_type == robot_type:
                return device
        return None
    
    def create_and_register_device(self, robot_type, **kwargs) -> Optional[DeviceBase]:
        """
        根据robot_type创建并注册设备
        ...
        while True:
            ...
            if isinstance(api_up.robot_type, int):
                ...
                if device:
                    ...
                    try:
                        device = self.create_and_register_device(robot_type)
                    except Exception as e:
