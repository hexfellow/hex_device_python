...
class MotorCommand:
    ...
    def __post_init__(self):
        ...
        if self.command_type == CommandType.BRAKE:
            ...
            if self.speed_command is not None or self.position_command is not None or self.torque_command is not None:
                raise ValueError("brake指令类型不应包含speed_command、position_command或torque_command")
            ...
            if self.brake_command is not None or self.position_command is not None or self.torque_command is not None:
                raise ValueError("speed指令类型不应包含brake_command、position_command或torque_command")
            if not isinstance(self.speed_command, list) or not all(isinstance(x, (int, float)) for x in self.speed_command):
            ...
            if self.brake_command is not None or self.speed_command is not None or self.torque_command is not None:
                raise ValueError("position指令类型不应包含brake_command、speed_command或torque_command")
            if not isinstance(self.position_command, list) or not all(isinstance(x, (int, float)) for x in self.position_command):
            ...
            if self.brake_command is not None or self.speed_command is not None or self.position_command is not None:
                raise ValueError("torque指令类型不应包含brake_command、speed_command或position_command")
            if not isinstance(self.torque_command, list) or not all(isinstance(x, (int, float)) for x in self.torque_command):
    ...
    @classmethod
    def create_position_command(cls, positions: List[float], pulse_per_rotation: np.ndarray = None) -> 'MotorCommand':
        """创建位置指令"""
        if pulse_per_rotation is None:
            return cls(command_type=CommandType.POSITION, position_command=positions)
        
        # trans
        trans_positions = (np.array(positions) + np.pi) / (2 * np.pi) * pulse_per_rotation
        return cls(command_type=CommandType.POSITION, position_command=trans_positions.tolist())
...
class MotorBase(ABC):
    ...
    def get_motor_state(self, motor_index: int) -> str:
        ...
        if not 0 <= motor_index < self.motor_count:
            raise IndexError(f"Motor index {motor_index} out of range [0, {self.motor_count})")
        with self._data_lock:
    ...
    def motor_command(self, command_type: CommandType, values: List[float]):
        ...
        if command_type == CommandType.BRAKE:
            ...
            if len(values) != self.motor_count:
                raise ValueError(f"Expected {self.motor_count} speed values, got {len(values)}")
            command = MotorCommand.create_speed_command(values)
        elif command_type == CommandType.POSITION:
            if len(values) != self.motor_count:
                raise ValueError(f"Expected {self.motor_count} position values, got {len(values)}")
            command = MotorCommand.create_position_command(values, self._pulse_per_rotation)
        elif command_type == CommandType.TORQUE:
            if len(values) != self.motor_count:
                raise ValueError(f"Expected {self.motor_count} torque values, got {len(values)}")
    ...
    def update_data(self, positions: List[float], velocities: List[float], 
                   torques: List[float], driver_temperature: List[float],
                   motor_temperature: List[float], voltage: List[float],
        ...
        if len(positions) != self.motor_count:
            raise ValueError(f"Expected {self.motor_count} positions, got {len(positions)}")
        if len(velocities) != self.motor_count:
            raise ValueError(f"Expected {self.motor_count} velocities, got {len(velocities)}")
        if len(torques) != self.motor_count:
            raise ValueError(f"Expected {self.motor_count} torques, got {len(torques)}")
        if len(driver_temperature) != self.motor_count:
            raise ValueError(f"Expected {self.motor_count} driver_temperatures, got {len(driver_temperature)}")
        if len(motor_temperature) != self.motor_count:
            raise ValueError(f"Expected {self.motor_count} motor_temperatures, got {len(motor_temperature)}")
        if len(voltage) != self.motor_count:
            raise ValueError(f"Expected {self.motor_count} voltages, got {len(voltage)}")
        
        if pulse_per_rotation is not None and len(pulse_per_rotation) != self.motor_count:
            raise ValueError(f"Expected {self.motor_count} pulse_per_rotation values, got {len(pulse_per_rotation)}")
        
        if wheel_radius is not None and len(wheel_radius) != self.motor_count:
            raise ValueError(f"Expected {self.motor_count} wheel_radius values, got {len(wheel_radius)}")
        ...
        with self._data_lock:
            ...
            # trans
            self._positions = np.array(positions)  / self._pulse_per_rotation * 2 * np.pi - np.pi
            ...
            if error_codes is not None:
                if len(error_codes) != self.motor_count:
                    raise ValueError(f"Expected {self.motor_count} error codes, got {len(error_codes)}")
    ...
    def update_motor_data(self, motor_index: int, position: float, velocity: float,
                         torque: float, driver_temperature: float,
                         motor_temperature: float, voltage: float,
        ...
        with self._data_lock:
            ...
            if self._target_command:
                if self._target_command.command_type == CommandType.BRAKE:
                    ...
                    status['target_velocity'] = self._target_command.speed_command[motor_index]
                elif self._target_command.command_type == CommandType.POSITION:
                    status['target_position'] = self._target_command.position_command[motor_index]
                elif self._target_command.command_type == CommandType.TORQUE:
                    status['target_torque'] = self._target_command.torque_command[motor_index]
            else:
    ...
    def _construct_target_msg(self) -> public_api_types_pb2.MotorTargets:
        ...
        with self._command_lock:
            if self._target_command is None:
                raise ValueError("Construct down msg failed, No target command")
